...
static const char BASE64_ALPHABET[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_";

// Constants to convert the temperature values to a base64 string
static const float TEMPERATURE_OFFSET = 0;
static const float TEMPERATURE_SCALE = 100.0;
...
void Database::appendDataToJSON(const temperatureData* data) {
    // Clears the previous data stored in the payload array
    payload = "";
    // Append the sensors' data to the payload string
    for (int i = 0; i < MLX90640_FRAME_SIZE; i++) {

        if (encodeBase64(data->temperaturePixels[i], encodedValue, 
                          TEMPERATURE_OFFSET, TEMPERATURE_SCALE)) {
            LogErrorln("Value ", data->temperaturePixels[i], " truncated to ", encodedValue);
        }
...
void Database::sendData(TemperatureDataBuffer* dataBuffer) {
    ...
    // Otherwise, we keep filling the JSON buffer with the data from the main buffer
        ...
        const temperatureData* sample = dataBuffer->getSample();
...
bool Database::encodeBase64(float input, char* output, float offset, float scale) {

    bool truncated = false;

    // Convert the float value to a int value (using location and scale transformation)
    int value = (int) ((input + offset) * scale);

    // If the value is out of the desired range (0-4095), we truncate it
    if (value < 0) {
        value = 0;
        truncated = true;
    } else if (value > 4095) {
    ...
    output[0] = BASE64_ALPHABET[(value >> 6) & 0b00111111];  // Extract and encode the first 6 bits
    output[1] = BASE64_ALPHABET[value & 0b00111111];         // Extract and encode the next 6 bits
    output[2] = '\0';                                        // Add the null terminator

    // Return a flag indicating if the value was truncated (if it was forced to be in the range)
    return truncated;
}