/*
    ...
    * This module handles the buffer that stores the data collected from the thermal camera.
 ...
 * temperaturePixels: array of temperatures of the pixels of the thermal camera
 */
...
/**
 * Class that handles the buffer that stores the data collected from the thermal camera.
 * The buffer consists of an array of temperatureData structs and some indexes to keep track of
 * the buffer state. It also provides functions to add and get samples from the buffer,
 * handle buffer capacity and indexes.
 * For debug purposes, it also provides functions to print the buffer state and dump
 * its content.
 * 
 * @param buffer the array of temperatureData structs that stores the collected data
...
public:
    ...
    // Create a buffer based on the temperatureData struct
    temperatureData buffer[BUFFER_CAPACITY];
